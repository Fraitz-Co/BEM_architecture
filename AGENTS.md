...
8. **Um branch só, e ele se chama `main`.** Se existe `main`, `master` não existe. Ao criar repo: `git init -b main`. Cuidado, porque a falha é silenciosa: `git push origin master` num repo cujo padrão é `main` não dá erro, cria o branch errado e devolve sucesso. Ver `README.md` → "Um branch só, e ele se chama `main`".
